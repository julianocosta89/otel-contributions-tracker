...
| `reports/datadog-monthly-report.json` | `scripts/send-monthly-report.mjs` | Monthly (GitHub Actions, 1st of month) |
...
### reports/datadog-monthly-report.json

Snapshot of the prior month's headline numbers for the Datadog Inc. monthly email report (`scripts/send-monthly-report.mjs`). Only overwritten after a successful send, so month-over-month deltas stay correct even if a run fails partway through.

```json
{
  "month": "2026-06",
  "totalContributions": 2270,
