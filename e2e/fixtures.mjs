...
// theme.js reads localStorage at startup, so this must run before the first
// page.goto() in a test — it seeds the value via an init script rather than
// setting it post-load, which wouldn't apply until a real page reload.
export async function setTheme(page, theme) {
  await page.addInitScript(t => localStorage.setItem('otel-theme', t), theme);
}
