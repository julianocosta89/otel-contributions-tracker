...
The **Maintainers** and **Approvers** tiles are `<button>`s (`#org-modal-maintainer-tile` / `#org-modal-approver-tile` in `index.html`): clicking one filters the contributor list below to exactly the people behind that tile's count. The filter matches the tile's own definition — `roleFor()` (highest role org-wide), so a maintainer anywhere never also appears under Approvers. A colored chip (`.org-role-tile`-styled, colored like the matching role badge via `ROLE_STYLE`) appears next to the "Contributors" heading showing `Role  shown / total`; clicking it, or clicking the active tile again, clears the filter. Clicking the other tile switches filters directly. Tiles with a count of 0 are `disabled` and inert. Filter state is modal-local: it resets on every (re)open and never touches the URL hash, so deep links and browser Back stay stable. Rows keep their original rank in the full list (a maintainer ranked 3rd still shows `3`), and the active/inactive divider still splits the filtered list at the threshold. Styling lives in `css/app.css` (`.org-role-tile`, hover outline + `aria-pressed` accent per role color).
