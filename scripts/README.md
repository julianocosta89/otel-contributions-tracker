...
- Among multiple current entries, the one with the latest `from` date wins.
- Later-numbered files override earlier ones when the same handle appears in both.
...
```
1. fetch-data.mjs            # required by fetch-github-companies.mjs
2. fetch-affiliations.mjs    # required by fetch-github-companies.mjs
